...
```
 #10 comparision 2018 GZL and GZB (dont think I need this chunk because VK did all the graphs, will check back)
...
#11 QQPLOTS per parameter for each station 15m
...
(qqplotspc <- ggplot(subset_dfspc, aes(sample = Value)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~station, scales = "free") +  # One plot per station
  labs(
...
unique(subset_dfDO$station)
#save
ggsave(plot = qqplotspc, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotspc.svg"),
       width = 14, height = 10)

#ph
parameter_to_plot <- "ph"
subset_dfph <- combined_df_long[combined_df_long$Parameter == parameter_to_plot, ]
(qqplotph <-ggplot(subset_dfph, aes(sample = Value)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~station, scales = "free") +  # One plot per station
  labs(
  ...
  theme_minimal())
#save
ggsave(plot = qqplotph, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotph.svg"),
       width = 14, height = 10)

#fluor
parameter_to_plot <- "fluorescence"
subset_dfflu <- combined_df_long[combined_df_long$Parameter == parameter_to_plot, ]
(qqplotfluor <-ggplot(subset_dfflu, aes(sample = Value)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~station, scales = "free") +  # One plot per station
  labs(
  ...
  theme_minimal())
#save
ggsave(plot = qqplotfluor, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotfluor.svg"),
       width = 14, height = 10)
...
(qqplotturb <-ggplot(subset_dfflu, aes(sample = Value)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~station, scales = "free") +  # One plot per station
  labs(
  ...
  theme_minimal())
#save
ggsave(plot = qqplotturb, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotturb.svg"),
       width = 14, height = 10)

#water temp 
parameter_to_plot <- "watertemperature"
subset_dfWT <- combined_df_long[combined_df_long$Parameter == parameter_to_plot, ]
(qqplotWT <-ggplot(subset_dfflu, aes(sample = Value)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~station, scales = "free") +  # One plot per station
  labs(
...
#save
ggsave(plot = qqplotWT, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotWT.svg"),
       width = 14, height = 10)
...
QQplots 1 hr averaged
```{r}

library(ggplot2)

# Define the parameter you want to plot
parameter_to_plot <- "dissolvedoxygen"

# Subset the data for the chosen parameter
subset_dfDO <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]

# Check if the subset contains data
if (nrow(subset_dfDO) > 0) {
  # Generate QQ plots for the chosen parameter across all stations
  qqplotdo1hr <- ggplot(subset_dfDO, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
  ...
  warning("The subset for the chosen parameter is empty. Check your data or parameter name.")
}


#how to save remaning QQplots change filename and path
ggsave(plot = qqplotdo1hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotdo1hr.svg"),
       width = 14, height = 10)

##SPC
parameter_to_plot <- "spc"
subset_dfspc <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]
qqplotspc1hr <- ggplot(subset_dfspc, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
ggsave(plot = qqplotspc1hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotspc1hr.svg"),
       width = 14, height = 10)

#ph
parameter_to_plot <- "ph"
subset_dfph <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]
qqplotph1hr <- ggplot(subset_dfph, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
#save
ggsave(plot = qqplotph1hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotph1h.svg"),
       width = 14, height = 10)

#fluor
parameter_to_plot <- "fluorescence"
subset_dfflu <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]
qqplotflu1h <- ggplot(subset_dfflu, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
#save
ggsave(plot = qqplotflu1h, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotfluor1h.svg"),
       width = 14, height = 10)


##turb
parameter_to_plot <- "turbidity"
subset_dfturb <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]
qqplotturb1h <- ggplot(subset_dfturb, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
#save
ggsave(plot = qqplotturb1h, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotturb1h.svg"),
       width = 14, height = 10)

#water temp 
parameter_to_plot <- "watertemperature"
subset_dfWT <- combined_df1h[combined_df1h$Parameter == parameter_to_plot, ]
qqplotwt1h <- ggplot(subset_dfWT, aes(sample = hourlyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
#save
ggsave(plot = qqplotwt1h, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotWT1h.svg"),
       width = 14, height = 10)
...
24hr averaged QQplots
```{r}
###missing or NA values
combined_df24h <- na.omit(combined_df24h)
...
# Subset the data for the chosen parameter
subset_dfDO24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]

  # Generate QQ plots for the chosen parameter across all stations
  qqplotdo24hr <- ggplot(subset_dfDO24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
    ...
    theme_minimal()
print(qqplotdo24hr)
ggsave(plot = qqplotdo24hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotdo24hr.svg"),
       width = 14, height = 10)
...
# Subset the data for the chosen parameter
subset_dfspc24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]

  # Generate QQ plots for the chosen parameter across all stations
  qqplotspc24hr <- ggplot(subset_dfspc24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
ggsave(plot = qqplotspc24hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotspc24hr.svg"),
       width = 14, height = 10)
...
# Subset the data for the chosen parameter
subset_dfph24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]

  # Generate QQ plots for the chosen parameter across all stations
  qqplotph24hr <- ggplot(subset_dfph24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
ggsave(plot = qqplotph24hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotph24hr.svg"),
       width = 14, height = 10)
...
# Subset the data for the chosen parameter
subset_dfflu24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]

  # Generate QQ plots for the chosen parameter across all stations
  qqplotflu24hr <- ggplot(subset_dfflu24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
ggsave(plot = qqplotflu24hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotflu24hr.svg"),
       width = 14, height = 10)
...
# Subset the data for the chosen parameter
subset_dfturb24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]

  # Generate QQ plots for the chosen parameter across all stations
  qqplotturb24hr <- ggplot(subset_dfturb24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
ggsave(plot = qqplotturb24hr, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotturb24hr.svg"),
       width = 14, height = 10)

###didnt work###
#watertemp
parameter_to_plot <- "watertemperature"
subset_dfWT24h <- combined_df24h[combined_df24h$Parameter == parameter_to_plot, ]
qqplotwt24h <- ggplot(subset_dfWT24h, aes(sample = dailyavg)) +
    stat_qq() +
    stat_qq_line(color = "red") +
    facet_wrap(~station, scales = "free") +  # One plot per station
    labs(
...
print(qqplotwt24h)
#save
ggsave(plot = qqplotwt24h, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotWT24h.svg"),
       width = 14, height = 10)


```

#12 Vivian's timeseries code 15m, 1HR, 24HR
...
plot_acf_pacf <- function(data, station, parameter) {
  ts_data <- data %>%
    filter(station == station) %>%
    arrange(date_time) %>%
    pull({{ parameter }})
  
  ts_obj <- ts(ts_data)  # Convert to time series object
  
  # ACF plot
  acf_plot <- ggAcf(ts_obj) + 
    ggtitle(paste("ACF -", station, parameter, "- 15min")) +
    theme_minimal()
  
  # PACF plot
  pacf_plot <- ggPacf(ts_obj) + 
    ggtitle(paste("PACF -", station, parameter, "- 15min")) +
    theme_minimal()
  
  list(ACF = acf_plot, PACF = pacf_plot)
}

# Parameters and stations
parameters <- c("dissolvedoxygen", "spc", "ph", "watertemperature", "turbidity", "fluorescence")
stations <- unique(combined_df$station)

# Loop through stations and parameters
for (station in stations) {
  for (parameter in parameters) {
    # Generate plots
    plots <- plot_acf_pacf(combined_df, station, parameter)
...
# Set desktop save path explicitly
desktop_path <- "C:/Users/nelsonm/Desktop/ACF_PACF_Plots"

# Create the directory
dir.create(desktop_path, showWarnings = TRUE, recursive = TRUE)
...
plot_acf_pacf <- function(data, station, parameter) {
  ...
  # ACF plot
  acf_plot <- ggAcf(ts_obj) + 
    ggtitle(paste("ACF -", station, parameter, "- 24h")) +
    theme_minimal()
  
  # PACF plot
  pacf_plot <- ggPacf(ts_obj) + 
    ggtitle(paste("PACF -", station, parameter, "- 24h")) +
    theme_minimal()
...
# Parameters and stations
parameters <- c("dissolvedoxygen", "spc", "ph", "watertemperature", "turbidity", "fluorescence")
stations <- unique(combined_df24$station)
...
for (station in stations) {
  for (parameter in parameters) {
    # Generate plots
    plots <- plot_acf_pacf(combined_df24, station, parameter)
    
    # Print plots to screen
    print(plots$ACF)
    print(plots$PACF)
    
   acf_file <- file.path(desktop_path, paste0("ACF_", station, "_", parameter, ".svg"))
    pacf_file <- file.path(desktop_path, paste0("PACF_", station, "_", parameter, ".svg"))
    ggsave(acf_file, plots$ACF, width = 14, height =10)
    ggsave(pacf_file, plots$PACF, width = 14, height = 10)
  }
}
...
#14 absolute diff plots
```{r}


#Absolute diff plots tried way back with Rosie
