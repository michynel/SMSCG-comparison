...
##GZL
df_GZLediALL <- read_csv("https://portal.edirepository.org/nis/dataviewer?packageid=edi.1177.5&entityid=bb661e9298f4c3c1aa3a875338c7c6a1")
view(df_GZLediALL)
##GZB
...
df_GZLediALL <- df_GZLediALL %>% mutate(date_time = paste(date, time)) %>% #combine the date and time columns
...
## last one needed dates cleaned GZL 2023, this one does have the seconds in the original datetime
 ...
 df_GZBFINAL <- df_GZBFINAL %>% mutate(date_time = as.POSIXct(date_time, format = "%Y-%m-%d %H:%M:%S"))
...
plot <- ggplot(data, aes(x = x, y = y)) +
  ...
  labs(title = "Example Scatter Plot",  # Add title
...
df_GZBFINAL <- rearrange_columns(dfGZB, reference_order)
df_GZLediALL <- rearrange_columns(df_GZLediALL, reference_order)
