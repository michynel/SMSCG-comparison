...
## here is a code for all do not like the scale though decided to keep out
...
#plots <- lapply(parameters, function(param) {
 ...
 # subset_df <- combined_df_long[combined_df_long$Parameter == param, ]
  ...
  # Generate QQ plots for the chosen parameter across all stations
 #qqplotALL<- ggplot(subset_df, aes(sample = Value)) +
  #  stat_qq() +
   # stat_qq_line(color = "red") +
    #facet_wrap(~station, scales = "free") +  # One plot per station
    #labs(
...
#ggsave(plot = qqplotALL, filename = abs_path_emp("Water Quality/SMSCG/QQplots/qqplotALL.svg"),
       #width = 14, height = 10)
